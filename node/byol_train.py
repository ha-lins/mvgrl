...
def train(dataset, verbose=False):
    ...
    _, adj, diff, features, labels, idx_train, idx_val, idx_test = load(dataset)
    ...
    for epoch in range(nb_epochs):
        ...
        if loss < best:
            ...
            best_t = epoch
            cnt_wait = 0
            torch.save(model.state_dict(), 'byol_model.pkl')
        else:
            cnt_wait += 1
    ...
    model.load_state_dict(torch.load('byol_model.pkl'))
